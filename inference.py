...
def top_k_p(pred_probs, k=1, p=None):
    pred_probs_cat = pred_probs.reshape((-1, pred_probs.shape[2]))
    if p is None:
        # top-k sampling
        assert k < pred_probs.shape[2], 'k larger than vocabulary size'
        values_topk, indices_topk = torch.topk(pred_probs_cat, k)
        sampled_idxs = torch.multinomial(values_topk, 1)
        new_token_ids = torch.gather(indices_topk, dim=1, index=sampled_idxs).reshape((pred_probs.shape[0], -1))
        new_token_probs = torch.gather(values_topk, dim=1, index=sampled_idxs).reshape((pred_probs.shape[0], -1))
    else:
        # nucleus sampling
        sorted_probs, sorted_idxs = pred_probs_cat.sort(descending=True)
        probs_cum = sorted_probs.cumsum(dim=1)
        sorted_probs[probs_cum > p] *= 1e-10
        sampled_idxs = torch.multinomial(sorted_probs, 1)
        new_token_ids = torch.gather(sorted_idxs, dim=1, index=sampled_idxs).reshape((pred_probs.shape[0], -1))
        new_token_probs = torch.gather(sorted_probs, dim=1, index=sampled_idxs).reshape((pred_probs.shape[0], -1))
    return new_token_probs, new_token_ids
    ...
    for iteration, batch in tqdm(enumerate(data_loader, 0), total=len(data_loader)):
        ...
        with torch.no_grad():
            ...
            for l, (low, high) in enumerate(config.boundaries, 1):
                ...
                # for top-k / nucleus
                top_k_val = 1  # top-k sampling: default is 1
                top_p_val = None  # p value for nucleus sampling: default is None, can get value between (0,1)
                pred_token_probs, pred_token_ids = top_k_p(pred_probs, k=top_k_val, p=top_p_val)
                ...
                for step in range(1, total_steps):
                    ...
                    pred_probs = F.softmax(pred_scores[:, num_regions:, :], dim=-1)
                    pred_probs[:, low - 1:, EOS] *= eos_penalizers[l - 1]
                    new_token_probs, new_token_ids = top_k_p(pred_probs, k=top_k_val, p=top_p_val)
        ...
        boundaries = data_loader.dataset.boundaries
        for level, preds_per_level in enumerate(pred_list, 1):
            l = boundaries[level - 1][0]
            h = boundaries[level - 1][1]
            for batch_id, image_id in enumerate(image_ids):
                ...
                pred_len = len(tokenizer.encode(pred_per_level))
                if l <= pred_len <= h:  # consider only valid length captions
