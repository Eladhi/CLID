...
class FeatureExtractor:
    ...
    def _process_feature_extraction(
        ...
        for i in range(batch_size):
            ...
            # normalize bbox
            bbox[:, 0] /= im_infos[i]["width"]
            bbox[:, 1] /= im_infos[i]["height"]
            bbox[:, 2] /= im_infos[i]["width"]
            bbox[:, 3] /= im_infos[i]["height"]
            torch.clip(bbox, max=1.)
            bbox = torch.cat((bbox, (bbox[:, 2] - bbox[:, 0]).unsqueeze(1), (bbox[:, 3] - bbox[:, 1]).unsqueeze(1)), axis=1)
            info_list.append(
                {
                    "bbox": bbox.cpu().numpy(),
                    "num_boxes": num_boxes.item(),
                    "objects": scores[keep_boxes].cpu().numpy(),  #objects.cpu().numpy(),
    ...
    def extract_features(self):
        ...
        if os.path.isfile(image_dir):
            ...
            _chunks = self._chunks(files, self.args.batch_size)
            for chunk in tqdm(_chunks, total=int(len(files)/self.args.batch_size)):
